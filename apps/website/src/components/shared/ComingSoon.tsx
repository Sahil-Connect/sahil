
export const ComingSoon = () => {
  return (
    <div className="min-h-screen">
        <div className="flex flex-col items-center justify-center">
            <h1 className="mt-10 text-xl text-center font-semibold">
                We&#39;re under construction. Please check back for an update soon.
            </h1>
            <div className="flex items-center gap-4 mt-4">
                <h3 className="text-lg text-center font-medium">Thank you for your patience!</h3>
                <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 48 48">
                    <g fill="currentColor" fill-rule="evenodd" clip-rule="evenodd">
                        <path d="M24 40c8.837 0 16-7.163 16-16S32.837 8 24 8S8 15.163 8 24s7.163 16 16 16m0 2c9.941 0 18-8.059 18-18S33.941 6 24 6S6 14.059 6 24s8.059 18 18 18"/>
                        <path d="M21.286 21.249c.107.398.146.835.016 1.193a.5.5 0 0 1-.728.257a7.15 7.15 0 0 1-.766-.557l-.195-.16c-.18-.146-.354-.288-.534-.423c-.502-.375-.9-.573-1.245-.556c-.362.018-.8.271-1.362.715c-.159.125-.329.267-.505.415c-.388.324-.808.675-1.207.931a.5.5 0 0 1-.766-.352c-.05-.365.026-.821.169-1.267a5.56 5.56 0 0 1 .695-1.416c.62-.907 1.626-1.762 2.973-1.706c1.3.055 2.264.817 2.844 1.647c.292.416.498.863.61 1.279m5.345 0c-.108.398-.147.835-.017 1.193a.5.5 0 0 0 .729.257c.262-.158.521-.36.765-.557l.196-.16c.18-.146.354-.288.534-.423c.501-.375.9-.573 1.244-.556c.363.018.8.271 1.363.715c.158.125.328.267.505.415c.387.324.807.675 1.207.931a.5.5 0 0 0 .765-.352c.05-.365-.026-.821-.168-1.267a5.56 5.56 0 0 0-.695-1.416c-.62-.907-1.627-1.762-2.974-1.706c-1.3.055-2.263.817-2.844 1.647a4.172 4.172 0 0 0-.61 1.279M24 37c-4.038 0-7.538-3.048-9.258-7.5a17.99 17.99 0 0 1-.05-.129C14.246 28.18 15.2 27 16.473 27h15.055c1.273 0 2.228 1.18 1.78 2.371a17.99 17.99 0 0 1-.049.129C31.538 33.952 28.038 37 24 37m7.276-7.928c-.3.728-.649 1.396-1.035 2H17.759a12.46 12.46 0 0 1-.913-1.715v-.285z"/>
                        <path d="M31.272 33.072c.227-.295.444-.603.65-.923a14.6 14.6 0 0 0 1.385-2.778c.383-1.017-.258-2.027-1.247-2.3a2 2 0 0 0-.533-.071H16.473a2 2 0 0 0-.533.072a1.89 1.89 0 0 0-1.094.797a1.662 1.662 0 0 0-.153 1.502a17.99 17.99 0 0 0 .322.79c.31.705.666 1.37 1.063 1.988c.206.32.423.628.65.922C18.602 35.502 21.168 37 24 37c2.831 0 5.398-1.498 7.272-3.928m.004-4a12.776 12.776 0 0 1-1.035 2H17.759a12.46 12.46 0 0 1-.913-1.715v-.285z"/>
                    </g>
                </svg>
            </div>
        </div>
    </div>
  )
}